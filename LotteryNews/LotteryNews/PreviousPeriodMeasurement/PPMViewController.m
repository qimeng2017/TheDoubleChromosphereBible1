...
-(void)setupBasic{
    ...
    //顶部
    LTPreHeaderView *headerView = [[LTPreHeaderView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.frame), HeaderViewHeight)];
    headerView.delegate = self;
    [self.view addSubview:headerView];
    _headerView = headerView;
    self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, (HeaderViewHeight+SegmentViewHeight), kScreenWidth, CGRectGetHeight(self.view.frame)-(HeaderViewHeight+SegmentViewHeight)-64) style:UITableViewStylePlain];
...
- (void)segmentView:(NSArray *)arr{
    ...
    kWeakSelf(self);
    _segement = [[LiuXSegmentView alloc]initWithFrame:CGRectMake(0, 60, kScreenWidth, SegmentViewHeight) segmentType:@"play" titles:arr clickBlick:^(NSInteger index) {
...
- (void)PPMTableViewCell:(PPMTableViewCell *)cell onLookClick:(PPMDataModel *)sender indexPath:(NSIndexPath *)indexPath{
    LNAlertView *alertView = [[LNAlertView alloc]initWithTitle:@"提示" message: @"您需要登录后才可以查看该内容" cancelButtonTitle:@"取消"];
