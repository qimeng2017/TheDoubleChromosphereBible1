...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:LOTTORY_REVIEW_STATUS]) {
        [self fiveStarHighPraisePopupView];
    }
}
//开发人员可见
- (void)debufTest{
...
//好评
- (void)fiveStarHighPraisePopupView{
    NSString *userid = UserDefaultObjectForKey(LOTTORY_AUTHORIZATION_UID);
    [[UserStore sharedInstance]fiveStarHighPraise:userid sucess:^(NSURLSessionDataTask *task, id responseObject) {
        NSNumber *codeNum = [responseObject objectForKey:@"code"];
        NSInteger code = [codeNum integerValue];
        ...
        if (code == 1) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self alertView:title message:message];
            });
        }
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
...
- (void)presentAppstory{
    ...
    NSString *reviewURL = templateReviewURLiOS8 ;
    
    NSString *url = [reviewURL stringByReplacingOccurrencesOfString:@"appid" withString:appid];
